...
//            quaternion = simd_quatf(float4x4(rotation: rotation))
            ...
            // Rotate this fucking thing instead of the model?? Don't just push the z but also y rotates
            let delta: Float = 0.003
            
            let rot: float3 = player.moveStates.reduce(into: [0, 0, 0]) { result, key in
                switch key {
                case .forward:
                    result.x += delta
                case .backwards:
...
//            assert(rotAlt == rot)
        ...
        let newRotMat = float4x4(rotation: rot)
        quaternion = simd_quatf(newRotMat) * quaternion
//        }
    }
